 ...
 *
 * Copyright 2017 Noxxxious - Modified to work with State Notifier
...
static struct notifier_block fsync_state_notif;

#define DYN_FSYNC_VERSION_MAJOR 2
#define DYN_FSYNC_VERSION_MINOR 5

/*
 * fsync_mutex protects dyn_fsync_active during state notifier suspend / resume
 * transitions
 */
...
static int dyn_fsync_init(void)
{
	...
	fsync_state_notif.notifier_call = state_notifier_callback;
	if (state_register_client(&fsync_state_notif))
...
{
	state_unregister_client(&fsync_state_notif);
